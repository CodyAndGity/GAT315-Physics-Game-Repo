...
public class PhysicsRaycast : MonoBehaviour
{
	...
	[SerializeField] public Type type = Type.Ray;      // The type of cast to perform
	[SerializeField] public float size = 1;              // The size of the sphere or box (diameter/width)
	[SerializeField] public float distance = 2;          // How far to cast
	[SerializeField] public LayerMask layerMask = Physics.AllLayers;  // Which layers to include in the cast
	...
	public void PerformRaycast(Type type)
	{
	...
	private void OnDrawGizmosSelected()
	{
		...
		if (raycastHits != null)
		{
			...
			foreach (var hit in raycastHits)
			{
				...
				{
					rb.AddForceAtPosition(transform.forward, hit.point, ForceMode.Force); // Apply an impulse force at the hit point
